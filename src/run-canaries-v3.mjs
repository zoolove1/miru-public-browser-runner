...
async function closeContext(context) {
  await Promise.race([context.close().catch(() => {}), new Promise((resolve) => setTimeout(resolve, 5_000))]);
}
...
await run('youtube', async () => {
  ...
  const context = await browser.newContext({
    ...
    locale: 'en-US',
    extraHTTPHeaders: { Referer: appReferrer },
  });
  const page = await context.newPage();
  ...
  try {
    await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: 60_000, referer: appReferrer });
    ...
    return { ok: true, status: 'PUBLIC_YOUTUBE_PLAYBACK_VERIFIED', videoId, targetUrl, appOrigin, appReferrer, before, after, advancedBy };
  } finally {
    await closeContext(context);
  }
});
